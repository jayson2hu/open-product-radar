...
应用启动时读取管理员名单；更新环境配置并重启应用后，每次请求按新名单核对，旧会话不会保留已撤销的管理员权限。编辑账户仅处理内容，不能访问订单、预算、私人投递队列或启停来源。
...
首次记录只提供总量；24h/7d 增长需要后续持续采集与对应时间的早期记录。未启动常驻 Worker 时，等待不会自动补齐。最后一次尝试崩溃的任务在租约到期后的下一次处理进入失败状态，不再永久停留在执行中。手动重试会重新占用实际请求预算。

本地采集报告的 `cached_endpoint_count` 是本轮有响应缓存的不同地址数，不能当作精确 HTTP 请求次数；`request_budget_reserved` 是预留额度，也不等于实际请求数。兼容字段 `request_count` 保留原名称，以 `request_count_basis` 说明其口径。
...
独立台账必须带 `mode:'demo'|'production'`，并与备份模式一致。旧版无 mode 台账不能直接恢复，应从对应的当前数据库重新导出，或使用 `--current` 自动读取最新删除记录；不要手工猜测环境补字段。新备份文件和 manifest 请求仅所有者读写权限，Windows 部署还需核对目标目录 ACL。
