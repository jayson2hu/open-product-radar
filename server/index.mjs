...
import { getEntity, getEvent, getRelations } from './store.mjs';
  ...
  const server=http.createServer(async(req,res)=>{
    ...
    try{
      ...
      if(method==='GET'&&path==='/auth/github'){
        ...
        const client=process.env.GITHUB_CLIENT_ID;if(!client||!process.env.GITHUB_CLIENT_SECRET||!publicUrl)fail(503,'AUTH_UNCONFIGURED','GitHub 登录尚未配置');
      ...
      if(method==='GET'&&path==='/auth/github/callback'){
        if(mode!=='production'||!publicUrl||!process.env.GITHUB_CLIENT_ID||!process.env.GITHUB_CLIENT_SECRET)fail(503,'AUTH_UNCONFIGURED','GitHub 登录尚未配置');
        ...
        const tokenResponse=await fetch('https://github.com/login/oauth/access_token',{method:'POST',headers:{Accept:'application/json','Content-Type':'application/json'},body:JSON.stringify({client_id:process.env.GITHUB_CLIENT_ID,client_secret:process.env.GITHUB_CLIENT_SECRET,code,redirect_uri:`${publicUrl.replace(/\/$/,'')}${PREFIX}/auth/github/callback`}),signal:AbortSignal.timeout(15000)});
        ...
        if(!account){const uid=id('user');const admins=(process.env.RADAR_ADMIN_GITHUB_IDS||'').split(',');db.prepare('INSERT INTO users(id,name,email,role,email_opt_in,timezone,github_id,is_demo,unsubscribe_token,created_at) VALUES(?,?,?,?,?,?,?,?,?,?)').run(uid,profile.name||profile.login,profile.email||null,admins.includes(String(profile.id))?'admin':'user',0,'Asia/Shanghai',String(profile.id),0,token(),now());account=db.prepare('SELECT * FROM users WHERE id=?').get(uid);}
      ...
      if(method==='GET'&&['/feed','/repositories','/products','/entities'].includes(path)){
        ...
        const sort=url.searchParams.get('sort')||'growth',metric=period==='7d'?'delta_7d':'delta_24h';
        const where=["review_status='published'"],params=[];
        if(kind){where.push('kind=?');params.push(kind);}
        if(q){where.push("lower(name || ' ' || COALESCE(owner,'') || ' ' || description || ' ' || extra_json) LIKE ? ESCAPE '\\'");params.push('%'+q.replace(/[\\%_]/g, c=>'\\'+c)+'%');}
        if(topic&&topic!=='all'){where.push('topic=?');params.push(topic);}
        if(language&&language!=='all'){where.push('language=?');params.push(language);}
        const order=sort==='stars'?'stars DESC':sort==='recent'?'observed_at DESC':`${metric} DESC`;
        const limit=Math.max(1,Math.min(200,Math.trunc(Number(url.searchParams.get('limit'))||100)));
        const data=db.prepare(`SELECT entities.*,EXISTS(SELECT 1 FROM entity_evidence ee JOIN evidence ev ON ev.id=ee.evidence_id JOIN sources s ON s.id=ev.source_id WHERE ee.entity_id=entities.id AND s.status='blocked') AS metadata_blocked FROM entities WHERE ${where.join(' AND ')} ORDER BY ${order},name LIMIT ?`).all(...params,limit).map(row=>{if(row.metadata_blocked){row.description='来源权限已撤销，介绍暂不展示';row.original_description=null;row.extra_json='{}';}const extra=json(row.extra_json,{});delete row.extra_json;delete row.metadata_blocked;return {...row,...extra,is_demo:!!row.is_demo,featured:!!row.featured,tags:extra.tags||[]};});
        ...
        if(path==='/feed'){
          ...
          result.stats={repositories:db.prepare("SELECT count(*) n FROM entities WHERE kind='repository' AND review_status='published'").get().n,products:db.prepare("SELECT count(*) n FROM entities WHERE kind='product' AND review_status='published'").get().n,events:db.prepare("SELECT count(*) n FROM events WHERE review_status='published'").get().n,sources:result.collection.data_source_count};
      ...
      if(method==='GET'&&(match=path.match(/^\/evidence\/([^/]+)$/))){const e=db.prepare("SELECT e.* FROM evidence e LEFT JOIN sources s ON s.id=e.source_id WHERE e.id=? AND e.review_status='published' AND e.permission_status IN ('approved','permitted','demo') AND (s.id IS NULL OR s.status!='blocked')").get(decodeURIComponent(match[1]));if(!e)fail(404,'NOT_FOUND','证据不存在或已撤回');return reply({...e,is_demo:!!e.is_demo});}
      ...
      if(path.startsWith('/admin/')){
        ...
        if(method==='GET'&&path==='/admin/overview'){
          ...
          return reply({entities:db.prepare('SELECT id,kind,name,slug,review_status FROM entities ORDER BY name LIMIT 1000').all(),evidence:db.prepare('SELECT * FROM evidence ORDER BY fetched_at DESC LIMIT 200').all(),sources:db.prepare('SELECT * FROM sources').all(),jobs:db.prepare('SELECT * FROM jobs ORDER BY last_run_at DESC').all().map(j=>({...j,payload:json(j.payload_json,{})})),corrections:db.prepare('SELECT * FROM corrections ORDER BY created_at DESC LIMIT 100').all(),pending_events:db.prepare("SELECT * FROM events WHERE review_status!='published' ORDER BY observed_at DESC LIMIT 100").all().map(e=>({...getEvent(db,e),evidence:db.prepare('SELECT ev.* FROM evidence ev JOIN event_evidence ee ON ee.evidence_id=ev.id WHERE ee.event_id=?').all(e.id)})),relations:db.prepare('SELECT * FROM relations').all().map(r=>({...r,entity_name:db.prepare('SELECT name FROM entities WHERE id=?').get(r.entity_id)?.name,target_name:db.prepare('SELECT name FROM entities WHERE id=?').get(r.target_entity_id)?.name,evidence_ids:db.prepare('SELECT evidence_id FROM relation_evidence WHERE relation_id=?').all(r.id).map(e=>e.evidence_id)})),audit:db.prepare('SELECT * FROM audit ORDER BY created_at DESC LIMIT 100').all(),metrics:{...counts,events:counters,is_demo:mode==='demo',note:mode==='demo'?'演示使用情况，不能用于商业结论':'试点实际使用记录；付费与留存需按阶段、渠道、用户组分开解释'},costs:db.prepare('SELECT * FROM costs ORDER BY created_at DESC LIMIT 100').all(),orders:db.prepare('SELECT * FROM orders ORDER BY created_at DESC LIMIT 100').all(),budget:budgetStatus(),quality:{published_events:db.prepare("SELECT count(*) n FROM events WHERE review_status='published'").get().n,pending_events:db.prepare("SELECT count(*) n FROM events WHERE review_status='pending'").get().n,missing_evidence:db.prepare("SELECT count(*) n FROM events e WHERE e.review_status='published' AND NOT EXISTS(SELECT 1 FROM event_evidence ee JOIN evidence ev ON ev.id=ee.evidence_id WHERE ee.event_id=e.id AND ev.review_status='published')").get().n,deleted_evidence:db.prepare('SELECT count(*) n FROM deletion_tombstones').get().n},outbox:db.prepare('SELECT id,user_id,digest_id,status,created_at,last_error FROM outbox ORDER BY created_at DESC LIMIT 100').all(),mode});
        }
