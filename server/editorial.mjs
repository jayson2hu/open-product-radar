...
export function createEditorialRoutes({db,mode,audit,body,fail,textField,enumField,numField,safeURL,needAdmin,entityExists}) {
  ...
  function refs(value,{published=false,required=true}={}) {
    ...
    for(const valueId of value){const e=db.prepare('SELECT e.*,s.id source_record_id,s.status source_status,s.permission_status source_permission FROM evidence e LEFT JOIN sources s ON s.id=e.source_id WHERE e.id=?').get(valueId);if(!e||!e.source_record_id||['deleted','rejected','retracted'].includes(e.review_status)||['blocked','revoked'].includes(e.source_status))fail(409,'EVIDENCE_REQUIRED','证据不存在、来源被禁用或已撤回');if(published&&(e.review_status!=='published'||!['approved','permitted','demo'].includes(e.permission_status)||!['approved','permitted','demo'].includes(e.source_permission)))fail(409,'EVIDENCE_REQUIRED','事实判断必须引用已经审核且来源权限有效的证据');}
    return value;
  }
  ...
  return async function editorial({path,method,req,user,reply}) {
    ...
    if(method==='POST'&&path==='/admin/evidence'){
      const b=await body(req),entity=entityExists(b.entity_id),source=db.prepare('SELECT * FROM sources WHERE id=?').get(textField(b.source_id,'来源',{required:true,max:100}));if(!source)fail(404,'NOT_FOUND','请先创建来源记录');if(['blocked','revoked'].includes(source.status))fail(409,'SOURCE_BLOCKED','该来源已被禁用');
    ...
    if(method==='POST'&&(match=path.match(/^\/admin\/evidence\/([^/]+)\/review$/))){
      const evidenceId=decodeURIComponent(match[1]),e=db.prepare('SELECT * FROM evidence WHERE id=?').get(evidenceId);if(!e||e.review_status==='deleted')fail(404,'NOT_FOUND','证据不存在或已删除');const b=await body(req),status=enumField(b.status,['published','rejected'],'审核状态'),reason=textField(b.reason,'审核理由',{required:true,max:2000});const source=db.prepare('SELECT * FROM sources WHERE id=?').get(e.source_id);if(status==='published'&&(!['approved','permitted','demo'].includes(source?.permission_status)||['blocked','revoked'].includes(source.status)))fail(409,'SOURCE_PERMISSION','请先核对并批准来源权限');db.prepare('UPDATE evidence SET review_status=?,permission_status=?,reviewed_at=? WHERE id=?').run(status,source?.permission_status||'pending',now(),evidenceId);audit(user,'evidence_review',evidenceId,reason);reply(db.prepare('SELECT * FROM evidence WHERE id=?').get(evidenceId));return true;
    }
